...
import { Ankus, ANKUS_EXT_ID, ANKUS_ICON, StandardTermPart, ShareCode } from './ankusCommon';
/**
 * Initialization data for the jupyter_ankus extension.
 */
const extension = {
            ...
            fetch(Ankus.ankusURL +
                ...
                app.commands
                    ...
                    Promise.all([
                        ...
                        tracker.currentWidget.context.rename(codedat[ShareCode.CodePropertyName.name] + '.ipynb');
                        //notebook
                        const note = tracker.currentWidget.content;
                        //code content
                        const celldat = codedat[ShareCode.CodePropertyName.content];
                        //cell data list
    ...
    app.commands.addCommand(CommandID.insertCode.id, {
        ...
        execute: async (args) => {
            ...
            fetch(Ankus.ankusURL +
                ...
                //code content
                const celldat = response[ShareCode.CodePropertyName.content];
                //cell data list
                celldat.forEach((dat, idx) => {
                    //아래 셀 추가
                    NotebookActions.insertBelow(note);
