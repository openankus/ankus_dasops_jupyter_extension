...
let jpCommands: CommandRegistry;

export function createCommands(app: JupyterFrontEnd, mainmenu: IMainMenu) {
  ...
  app.commands.addCommand(CommandID.openCode.id, {
    ...
    icon: editIcon,
    execute: async args => {
      Ankus.ankusPlugin.openCodeEditor(args['id'] as number);
    }
  });
      ...
      if (
        ...
        if (token !== undefined) {
          try {
            ...
            const mn = await StandardTermPart.completerMenu(
              token.value,
              p.right,
              p.bottom
            );
...
export const codeContextMenu = (code: CodeObject): Menu => {
